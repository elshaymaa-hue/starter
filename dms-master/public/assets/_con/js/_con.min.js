...
! function(t, i) {
    i["true"] = t, window.conApp = {}, ! function(t) {
        ...
        "use strict";
        var i = function(i, n) {
            this.options = n, this.$card = t(i), this.$closeBtn = this.$card.find(".close"), this.$minimizeBtn = this.$card.find(".minimize"), this.$content = this.$card.find(".content"), this.$window = t(window)
        };
        i.DEFAULTS = {
            duration: 300
        }, i.prototype.init = function() {
    ...
    var i = function(i, n) {
        this.options = n, this.$chat = t(i), this.$window = t(window), this.$document = t(document), this.$chatForm = this.$chat.find(".send > form"), this.$msgNano = this.$chat.find(".messages .nano"), this.$msgCont = this.$msgNano.find("> .nano-content"), this.$msgInput = this.$chat.find("input[name=message]")
    };
    i.DEFAULTS = {
        msgDuration: 300,
        msgDemo: ""
        ...
        i.initLayer(), i.$chatForm.on("submit", function(t) {
            t.preventDefault(), i.sendMsg()
                ...
                // Clicando no usuario
               //Ativando Mensagem para usuario especifico
               var id = $(this).attr('id').split("_")[1];
               var path_foto = $('#userIdFoto_'+id).attr('src');
               var nome = $('#userIdNome_'+id).text();
               $("#header-chat-foto").attr('src', path_foto);
               $("#header-chat-foto").attr('alt', nome);
               $("#header-chat-username").text(nome)

               if($("#chat_with_user_"+id).length == 0) {
                  //Mensagem não existe , será criada aqui a div
                  var html = "";
                  var participant = id+"";
                  html += '<div class="list list_mensagem" style="display: none;" data-thread_id="not_created" id="chat_with_user_'+participant.replace(/,/g, '_')+'">';
                  ...
                  //mostrando div
                  $("#chat_with_user_"+id).show();

                  // Adicionando o recebedor input com o ID
                  $('#recipients').html("");
                  $('#recipients').append('<input type="checkbox" class="hide" name="recipients[]" value="'+id+'" checked>');
                  $("#subject").val("Chat com "+nome)
                }else{
                  //Mensagem existe e tem um thread id  
                    $("#chat_with_user_"+id).show();

                    var idt = $("#chat_with_user_"+id).attr('data-thread_id');
                    $("#thread_id").val(idt);


                }

                 // ativando mensagem de grupo usar selector com contêm... 
                $('.messages .list_mensagem > .nano').nanoScroller();
                $('.messages .list_mensagem > .nano').nanoScroller({ alwaysVisible: true, scroll: 'bottom' });
                ...
                // Clicando em Voltar
                // escondendo todas as divs mensage
                $('.list_mensagem').hide();
                //limpando campos do formularios
                $("#thread_id").val("");
                $("#message").val("");
...
tinymce.init({
  ...
  plugins: [
   ...
   toolbar2: 'print preview media | forecolor backcolor emoticons | speech',
   image_advtab: true,

 
  setup: function (editor) {
    editor.addButton('speech', {
      icon :'icon-microphone',
     // image: 'http://localhost:8000/assets/_con/images/microphone.png',

      tooltip: "Transcrição da Fala",
      onclick: function () {
...
function speach(){
    ...
    //fazer funcao de click clicado = true  chama transcriptor... clicado = false.. para transcriptor..
                ...
                for (var i = event.resultIndex; i < event.results.length; i++) {
                    if(event.results[i].isFinal){
                        ...
                        transcription = event.results[i][0].transcript;
                         ...
                         tinymce.activeEditor.execCommand('mceInsertContent', false, transcription);
                    }else{
                         tinymce.activeEditor.execCommand('mceInsertContent', false, transcription);
                    }
                }
